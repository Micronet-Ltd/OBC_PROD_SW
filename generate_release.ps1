...
# Build the testing app and copy it into the test software
cd .\OBCTestingApp
cmd.exe /c 'gradlew.bat clean'
cmd.exe /c 'gradlew.bat build'
cd ..
Copy-Item $CURR_DIR\OBCTestingApp\app\build\outputs\apk\release\app-release.apk -Destination $CURR_DIR\factory_software_v$TEST_VER\OBC_TEST_FILES\INSTALL_FILES\obc_testing_app.apk
