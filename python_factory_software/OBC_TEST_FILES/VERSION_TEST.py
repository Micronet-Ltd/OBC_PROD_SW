...
def Main(dict, configDict, update=True):
    ...
    if 'os_ver' not in configDict:
        print(Fore.RED + '** OS version', dict['TestFailDash'],
              ': Error, no OS version string in the configuration file. Contact MICRONET for the OS string' + Style.RESET_ALL)
        osExistBoolean = False
    if 'mcu_ver' not in configDict:
        print(Fore.RED + '** MCU version', dict['TestFailDash'],
              ': Error, no MCU version string in the configuration file. Contact MICRONET for the MCU string' + Style.RESET_ALL)
        mcuExistBoolean = False
    if 'fpga_ver' not in configDict:
        print(Fore.RED + '** FPGA version', dict['TestFailDash'],
              ': Error, no FPGA version string in the configuration file. Contact MICRONET for the FPGA string' + Style.RESET_ALL)
    ...
    if osExistBoolean:
        if deviceOSVer.strip().lower() == configDict['os_ver'].strip().lower():
            ...
            print(Fore.RED + ' ** OS version', dict['TestFailDash'], ': expected', configDict['os_ver'], 'got',
                  deviceOSVer, '. Burn correct OS version.' + Style.RESET_ALL)
            osBoolean = False
    ...
    if mcuExistBoolean:
        if deviceMCUVer.strip().lower() == configDict['mcu_ver'].strip().lower():
            ...
            print(Fore.RED + ' ** MCU version', dict['TestFailDash'], ': expected', configDict['mcu_ver'], 'got',
                  deviceMCUVer, '. Burn correct MCU version.' + Style.RESET_ALL)
            mcuBoolean = False
    ...
    if fpgaExistBoolean:
        if deviceFPGAVer.strip().lower() == configDict['fpga_ver'].strip().lower():
            ...
            print(Fore.RED + ' ** FPGA version', dict['TestFailDash'], ': expected', configDict['fpga_ver'], 'got',
                  deviceFPGAVer, '. Burn correct FPGA version.' + Style.RESET_ALL)
            fpgaBoolean = False
