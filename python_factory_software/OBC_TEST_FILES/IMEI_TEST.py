...
def Main(dict, update=True):
    ...
    if matchingDevice and correctTac:
        print(Fore.GREEN + dict['IMEIPass'].format(deviceIMEI) + Style.RESET_ALL)
        resultBool = True
    elif matchingDevice:
        print(Fore.RED + dict['IMEIFail'].format(deviceIMEI, scannedIMEI),
              ': matching device but IMEI should start with \'35483308\'' + Style.RESET_ALL)
    elif correctTac:
        print(Fore.RED + dict['IMEIFail'].format(deviceIMEI, scannedIMEI) + Style.RESET_ALL)
    else:
        print(Fore.RED + dict['IMEIFail'].format(deviceIMEI, scannedIMEI),
              ': incorrect label and IMEI should start with \'35483308\'' + Style.RESET_ALL)
