...
..\adb shell am startservice -a com.micronet.obctestingapp.action.SET_APN --es 'apn' '%apn%' --es 'mcc' '%mcc%' --es 'mnc' '%mnc%' --es 'name' '%name%' > nul
call color.bat 0a "** "
...
echo To change the APN settings, change the input values in "input\settings\test_options.dat" under APN and restart the test.
